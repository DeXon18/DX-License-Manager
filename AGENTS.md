...
- `Z:\DX-License-Manager-DEV` (/opt/web-projects/Development) → **ENTORNO DE DESARROLLO/BETA**. Aquí debes trabajar siempre, anclado a la rama `dev`.
- `Z:\DX-License-Manager` (/opt/web-projects/Production) → **PRODUCCIÓN**. Intocable. Anclado a `main`.
...
### 0.6.1 Gestión de Archivos No Rastreados o Desconocidos (Untracked)

**NUNCA** se eliminará por defecto ningún archivo o carpeta que aparezca como "untracked" o que sea desconocido para el agente, por muy prescindible que parezca (ej. volcados, carpetas temporales, logs antiguos).

Si el agente detecta archivos que ensucian el entorno y considera que deben borrarse, debe aplicar el siguiente protocolo de consulta:

```
⚠️ Archivos desconocidos detectados
He encontrado estos archivos/carpetas que no pertenecen al repositorio:
- [archivo 1]
- [carpeta 2]
¿Qué hacemos con ellos? ¿Los ignoro, los añado al .gitignore o los elimino?
```

Solo tras el permiso expreso del desarrollador se procederá a borrarlos.
...
- [2026-06-01] ERROR: Pérdida de conexión con el Daemon de Docker (`/var/run/docker.sock`) desde el dashboard del sistema tras ejecutar `docker compose up -d` para recrear el contenedor de PHP. → REGLA: Al recrear contenedores en un entorno LXC que acceden al socket de Docker, los permisos del socket en el host pueden restablecerse. Para solucionar la pérdida de acceso del usuario (`www-data`), se debe utilizar la asignación del grupo con GID `991` dentro del `Dockerfile` y añadir `group_add: ["991"]` en el servicio PHP correspondiente. Se debe conservar el socket con permisos estrictos `660`. NUNCA usar `chmod 666 /var/run/docker.sock` ni `chmod 777 /var/run/docker.sock`.
- [2026-06-02] ERROR: Riesgo crítico de corrupción en Producción al editar código en caliente en el entorno Beta, ya que ambos entornos compartían el mismo volumen de código (`./backend:/var/www/html`). → REGLA: **Arquitectura de Carpetas Separadas Obligatoria**. El entorno de Desarrollo/Beta DEBE ejecutarse en una carpeta física aislada (`/opt/web-projects/Development`) anclada a la rama `dev`, mientras que Producción (`/opt/web-projects/Production`) permanece aislada en `main`. NUNCA usar la misma carpeta física para contenedores de distintos entornos.
